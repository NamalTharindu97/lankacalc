...
const contentLabels: Record<Locale, Record<string, string>> = {
  en: { breadcrumb: "Breadcrumb", directAnswer: "Quick answer", instructions: "How to use it", formula: "Formula", example: "Worked example", assumptions: "Assumptions", exclusions: "Not included", mistakes: "Common mistakes", faq: "Frequently asked questions", related: "Related calculators", reviewed: "Reviewed", contributors: "Contributors", sources: "Sources", sourceKind: "Version-controlled calculation specification" },
  si: { breadcrumb: "මාර්ග සලකුණු", directAnswer: "කෙටි පිළිතුර", instructions: "භාවිත කරන ආකාරය", formula: "සූත්‍රය", example: "ගණනය කළ උදාහරණය", assumptions: "උපකල්පන", exclusions: "ඇතුළත් නොවේ", mistakes: "පොදු වැරදි", faq: "නිතර අසන ප්‍රශ්න", related: "අදාළ ගණක", reviewed: "සමාලෝචනය කළේ", contributors: "දායකයන්", sources: "මූලාශ්‍ර", sourceKind: "අනුවාද පාලිත ගණනය කිරීමේ පිරිවිතරය" },
  ta: { breadcrumb: "வழிசெலுத்தல்", directAnswer: "சுருக்கமான பதில்", instructions: "பயன்படுத்தும் முறை", formula: "சூத்திரம்", example: "கணக்கிட்ட உதாரணம்", assumptions: "கருதுகோள்கள்", exclusions: "சேர்க்கப்படாதவை", mistakes: "பொதுவான தவறுகள்", faq: "அடிக்கடி கேட்கப்படும் கேள்விகள்", related: "தொடர்புடைய கணிப்பான்கள்", reviewed: "மீளாய்வு", contributors: "பங்களிப்பாளர்கள்", sources: "ஆதாரங்கள்", sourceKind: "பதிப்புக் கட்டுப்பாட்டிலுள்ள கணக்கீட்டு விவரக்குறிப்பு" },
};
...
export default async function CalculatorPage({ params }: { params: Params }) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    {content ? <article className="mt-16 space-y-12 border-t pt-12">
      ...
      <section className="grid gap-6 rounded-xl border p-6 md:grid-cols-2">
        <div><h2 className="text-lg font-semibold">{labels.contributors}</h2>{content.contributors.map(contributor => <p className="mt-3 text-sm text-muted-foreground" key={`${contributor.name}-${contributor.role}`}><span className="font-medium text-foreground">{contributor.name}</span><br />{contributor.role}</p>)}</div>
        <div><h2 className="text-lg font-semibold">{labels.sources}</h2>{content.sources.map(source => <div className="mt-3 text-sm text-muted-foreground" key={source.repositoryPath}><p className="font-medium text-foreground">{source.title}</p><p>{labels.sourceKind}</p><code className="mt-1 block break-all text-xs">{source.repositoryPath}</code></div>)}</div>
      </section>
