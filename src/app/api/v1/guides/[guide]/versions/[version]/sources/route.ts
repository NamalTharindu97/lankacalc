...
import { addContentSource, listContentSources } from "@/server/guides/service";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const sources = await listContentSources(guide, version);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const source = await addContentSource(guide, version, body);
