...
Status: in progress. Construction quantity calculators (10.5) and the Business And Tax family (10.6) are code-complete. The lending family (10.1) is implemented including configurable rates, fees and insurance, early payment, lease deposits/residuals/balloons, and platform-observed bank rates with source and date via the CBSL AWPR rule; the electricity standard tariffs (10.2) are implemented including the domestic block tariff and the full non-domestic customer categories (religious, general purpose, government, industrial, hotel, street lighting, agriculture, and EVCS; Rate 1/2/3 supply classes with block, VDMC two-tier, time-of-use, and single-rate structures) with the `electricity-domestic-standard` and `electricity-non-domestic-standard` dev rules provisioned and published; fuel and solar (10.4) are implemented including the user price override and financing/payback; vehicle import (10.3) is implemented with the dated LC surcharge exemption and the provisioned dev rule. The remaining increments are data- and source-gated: historical tariff versioning (10.2), broader vehicle-import concessions (10.3), and licensed finance company lease rate observations (10.1). Production rule publication remains gated by independent source, formula, and rounding review.
...
Status: the standard CEB domestic tariff (categories 0-60, 61-180, and above 180, with fixed charges and a 2.5% SSCL) is implemented as a regulated calculator with an approved candidate specification, golden fixtures, and `docs/electricity-rule-sources.md`, and the `electricity-domestic-standard` dev rule is provisioned and published in the local database. The non-domestic categories (religious, general purpose, government, industrial, hotel, street lighting, agriculture, and EVCS; Rate 1/2/3 supply classes with VDMC two-tier, block, time-of-use, and single-rate structures) are implemented as the `electricity-non-domestic-bill` calculator with the `electricity-non-domestic-standard` dev rule, approved candidate spec, 12 golden fixtures, and sources recorded in `docs/electricity-rule-sources.md`; implementation is complete and production publication remains review-gated. Effective-dated rule versioning (multiple tariff dates with historical fallback) remains.
...
21. [x] Implement the CEB non-domestic electricity bill calculator (10.2): add the `electricity-non-domestic-bill` calculator with all Rate 1/2/3 customer categories (religious, general purpose, government, industrial, hotel, street lighting, agriculture, EVCS), block/v-dmc/tou/single-rate structures, 2.5% SSCL levy, category-driven field visibility, `electricity-non-domestic-standard` dev rule with golden fixtures, approved candidate spec, and localization keys.
