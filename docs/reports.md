...
Generation runs in-process as a background promise started by `createReport`; it is not yet a separately supervised worker:
...
```text
/api/v1/reports/{id}/download?expires={epochMs}&sig={hex}
```
...
REPORT_STUCK_JOB_TIMEOUT_MS = 15 * 60 * 1000   (15 minutes)
```
...
The sweep currently runs when an authenticated user lists reports; it is not independently scheduled. The protected rule desk provides read-only aggregate counts and oldest ages for queued, generating, stuck, ready, legacy completed, and failed jobs. It never returns report IDs, users, names, snapshots, PDFs, checksums, download metadata, or error text, and loading the dashboard does not run the sweep.
